...
class CST(nn.Module):
    def __init__(self, dim=28, stage=2, num_blocks=[2, 2, 2], sparse=False):
        ...
        self.fution = nn.Conv2d(dim*2, dim, 1, 1, 0, bias=False)

        # Sparsity Estimator
        if num_blocks==[2,4,6]:
            self.fe = nn.Sequential(Sparsity_Estimator(dim,expand=2,sparse=False),
                        Sparsity_Estimator(dim, expand=2, sparse=sparse))
        else:
            self.fe = Sparsity_Estimator(dim, expand=2, sparse=sparse)
